...
function changePinyin (value) {
  return pinyin(value, { removeSpace: true, removeTone: true })
}

function judageDataType (type) {
  return value => Object.prototype.toString.call(value) === `[object ${type}]`
}

const isObject = judageDataType('Object');
const isArray = judageDataType('Array');

function getFormData (data, arr, level, parentId) {
  if (isObject(data)) {
    Object.keys(data).forEach(key => {
      let item = {
        label: key,
        value: changePinyin(key) + level,
        children: [],
        parentId: parentId
      };
      arr.push(item);
      return getFormData(data[key], item.children, ++level, item.value);
    })
  }
  if (isArray(data)) {
    data.forEach(value => {
      let item = {
        label: value,
        value: changePinyin(value) + level,
        parentId: parentId
      }
      arr.push(item);
    })
...
let index = 0;
let currItem = formDataList[index++];
let listData = [];
while (currItem) {
    if (currItem) {
        if (currItem.children && currItem.children.length) {
            formDataList.push(...currItem.children);
        }
        listData.push({
            pid: currItem.parentId,
    ...
    ++index;
    currItem = formDataList[index];
}

export { formData, listData }